...
def buscar_columna_por_nombres(df, nombres):
    ...
    for columna in columnas:
        nombre_columna = str(columna).strip().lower()
...
def buscar_columna_asesor(df):
    ...
    return buscar_columna_por_nombres(df, posibles)


def es_fila_total(valor):
    """
    Determina si una fila corresponde al TOTAL.
    """
...
def calcular_porcentaje_nps_positivo(df):
    ...
    if not cols_nps:
        col_objetivo = df.columns[-1]
    else:
        col_objetivo = cols_nps[0]
    ...
    for val in df[col_objetivo].astype(str).str.lower():
...
def obtener_total_retenciones():

    filepath = "datos_retenciones.xlsx"
    ...
    col_rete = buscar_columna_rete_cta(df)
    col_beneficio = buscar_columna_beneficio(df)

    fila_total = None

    if col_asesor is not None:
    ...
    # Si no encontramos TOTAL, usamos la última fila
    if fila_total is None:
        fila_total = df.iloc[-1]

    resultado_rete = None
    resultado_beneficio = None

    if col_rete is not None:
        resultado_rete = normalizar_porcentaje(
            fila_total[col_rete]
        )

    if col_beneficio is not None:
        resultado_beneficio = normalizar_porcentaje(
            fila_total[col_beneficio]
        )

    return {
        "rete_cta": resultado_rete,
        "beneficio": resultado_beneficio
    }


def obtener_volumen_total_retenciones_desde_resumen(filepath):
    ...
    # Si existe una fila TOTAL, buscamos una columna
    # que represente volumen/total.
    ...
    # Si no existe una columna total clara,
    # sumamos columnas numéricas de asesores.
...
def obtener_datos_retenciones_totales():
    ...
    datos = obtener_total_retenciones()
