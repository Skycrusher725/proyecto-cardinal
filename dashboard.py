...
def mostrar_panel_asesor(
  ...
  """Panel exclusivo para Asesores con persistencia, filtrado inteligente y calendario visual mensual."""
    ...
    if not df_mi_asistencia.empty:
      col_fecha = next(
          (
              ...
              if any(
                  k in str(c).lower() for k in ["fecha", "dia", "date", "periodo"]
              )
          ),
          None,
      )
      ...
      if col_fecha:
        ...
        for _, row in df_mi_asistencia.iterrows():
          ...
          if pd.notna(f_val):
            ...
            estado_txt = row[col_estado] if col_estado else "Sin detalle"
        ...
        st.markdown(
            ...
            <div style="background-color: #161b22; padding: 12px; border-radius: 8px; border: 1px solid #30363d; margin-bottom: 15px;">
                <span style="color: #8b949e; font-size: 0.9rem;">👁️ <b>Vista del Operador:</b> Calendario mensual interactivo. Los colores reflejan el control de la supervisión. Selecciona una fecha abajo para desplegar el menú de detalles.</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if not valid_dts.empty:
          anio_actual = int(valid_dts.dt.year.iloc[0])
          mes_actual = int(valid_dts.dt.month.iloc[0])
        ...
        nombre_mes_str = nombres_meses[mes_actual]

        componentes_html = [
            f"""
            <style>
                .cal-box {{ background-color: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 15px; margin-bottom: 20px; font-family: sans-serif; }}
                .cal-header-title {{ text-align: center; color: #58a6ff; font-size: 1.2rem; font-weight: bold; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1px; }}
            ...
            <div class="cal-box">
                <div class="cal-header-title">📅 {nombre_mes_str} {anio_actual}</div>
                <div class="cal-grid">
          ...
          for dia in semana:
            if dia == 0:
              ...
              if info_d and info_d["color"] != "#21262d":
                ...
                txt_color = "#111111" if bg_color == "#d29922" else "#ffffff"
              else:
                bg_color = "#21262d"
                txt_color = "#c9d1d9"
        ...
        fechas_disponibles = df_mi_asistencia[col_fecha].astype(str).tolist()
        ...
        if fecha_elegida:
          info_dia = diccionario_asistencias.get(
              fecha_elegida.split(" ")[0], None
          )
